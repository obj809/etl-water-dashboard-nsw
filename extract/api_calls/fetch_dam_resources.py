...
OUTPUT_DIR = "data/input_data/dam_resources"
EXCLUDED_DAMS = ["BlueMountainsTotal", "401027"]  # Return API errors
...
def fetch_all_dam_resources(access_token, dams_list, start_date, end_date):
    ...
    for i, dam in enumerate(dams_list, 1):
        ...
        if dam_id in EXCLUDED_DAMS:
            print(f"  [{i}/{total}] {dam_name} ({dam_id})... skipped (excluded)")
            continue
...
def main():
    ...
    print("=" * 60)
    print("WaterInsights API - Fetch Dam Resources (Current Month)")
    print("=" * 60)
